...
hook OnPlayerConnect(playerid) {
	...
	pSpeedoTd[playerid][1] = CreatePlayerTextDraw(playerid, 552.500000, 344.000000, "Velocimetro");
	...
	pSpeedoTd[playerid][5] = CreatePlayerTextDraw(playerid, 503.000000, 401.000000, "IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII");
...
ShowPlayerSpeedoMeter(playerid)
{
	...
	PlayerTextDrawSetString(playerid, pSpeedoTd[playerid][5], "~w~IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII");
...
public UpdatePlayerSpeedo(playerid, vehicleid, Float:maxvel)
{
	...
	if(GLOBAL_VEHICLES[vehicleid][gb_vehicle_STATE] == VEHICLE_STATE_NORMAL)
	{
		...
		if(GLOBAL_VEHICLES[vehicleid][gb_vehicle_HEALTH] < MIN_VEHICLE_HEALTH)
		{
			...
			SendClientMessage(playerid, -1, "{CCCCCC}El motor del vehiculo esta demasiado golpeado.");
		}
	}
