...
new JobsRecordsBoards[][E_JobsRecordsBoards] = {
    ...
    {WORK_CARNICERO, 1415.8881, 1639.8406, -29.2564,    0.000000, 0.000000, 180.000000, 0, 0, INVALID_STREAMER_ID, INVALID_STREAMER_ID}
}; 
...
hook OnScriptInit() {
    for(new i = 0; i < sizeof JobsRecordsBoards; i ++) {
        ...
        SetDynamicObjectMaterialText(tmpobjid, 0, "{FFFFFF}MEJORES EMPLEADOS", 130, "Arial", 40, 1, 0x00000000, 0x00000000, 0);
        ...
        SetDynamicObjectMaterialText(JobsRecordsBoards[i][jobBoard_TOP_OBJECTID], 0, "- Sin datos", 130, "Segoe UI", 28, 1, 0xFFFFFFFF, 0x00000000, 0);
    }
}
...
SetWorkBoardsString(work, reset_string[], top_string[]) {
    for(new i = 0; i < sizeof JobsRecordsBoards; i ++) {
        if(JobsRecordsBoards[i][jobBoard_WORK] == work) {
            ...
            SetDynamicObjectMaterialText(JobsRecordsBoards[i][jobBoard_TOP_OBJECTID], 0, top_string, 130, "Segoe UI", 28, 1, 0xFFFFFFFF, 0x00000000, 0);
        }
    }
}
